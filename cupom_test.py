...
def verifica_campo_obrigatorio_objetoV(mensagem_esperada, venda):
    with pytest.raises(Exception) as excinfo:
        venda.dados_venda()
    the_exception = excinfo.value
    assert mensagem_esperada == str(the_exception)
...
DATAHORA_NULA = cupom.Venda(LOJA_COMPLETA, None, CCF_VENDA, COO_VENDA)
DATAHORA_VAZIA = cupom.Venda(LOJA_COMPLETA, "", CCF_VENDA, COO_VENDA)


def test_valida_datahora():
    verifica_campo_obrigatorio_objetoV("O campo data e hora do cupom são obrigatórios", DATAHORA_NULA)
    verifica_campo_obrigatorio_objetoV("O campo data e hora do cupom são obrigatórios", DATAHORA_VAZIA)

CCF_NULO = cupom.Venda(LOJA_COMPLETA, DATA_HORA_VENDA, None, COO_VENDA)
CCF_VAZIO = cupom.Venda(LOJA_COMPLETA, DATA_HORA_VENDA, "", COO_VENDA)

def test_valida_ccf():
    verifica_campo_obrigatorio_objetoV("O campo CCF do cupom é obrigatório", CCF_NULO)
    verifica_campo_obrigatorio_objetoV("O campo CCF do cupom é obrigatório", CCF_VAZIO)

COO_NULO = cupom.Venda(LOJA_COMPLETA, DATA_HORA_VENDA, CCF_VENDA, None)
COO_VAZIO = cupom.Venda(LOJA_COMPLETA, DATA_HORA_VENDA, CCF_VENDA, "")

def test_valida_coo():
    verifica_campo_obrigatorio_objetoV("O campo COO do cupom é obrigatório", COO_NULO)
    verifica_campo_obrigatorio_objetoV("O campo COO do cupom é obrigatório", COO_VAZIO)